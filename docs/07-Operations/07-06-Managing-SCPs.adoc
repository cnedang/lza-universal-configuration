...
LZA defines SCPs in separate JSON files that you reference in the `serviceControlPolicies` section of the `organization-config.yaml` file. This modular approach provides several advantages:
...
4. *Configure in Organization Config*:
   * Add a reference to your new SCP in the `serviceControlPolicies` section of the `organization-config.yaml` file
